...
import { roundValue } from "../../components/RepositoryItem";
    ...
    it("renders repository information correctly", () => {
      ...
      repositoryItems.forEach((repo, i) => {
        ...
        expect(repo).toHaveTextContent(
          roundValue(repositories.edges[i].node.forksCount)
        );
        expect(repo).toHaveTextContent(
          roundValue(repositories.edges[i].node.stargazersCount)
        );
        expect(repo).toHaveTextContent(
          roundValue(repositories.edges[i].node.ratingAverage)
        );
        expect(repo).toHaveTextContent(
          roundValue(repositories.edges[i].node.reviewCount)
        );
      });
    });
  });
